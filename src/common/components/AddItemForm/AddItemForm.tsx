...
export const AddItemForm = React.memo(function ({ addItem, disabled = false }: PropsType) {
  ...
  const addItemHandler = () => {
    if (title.trim() !== "") {
      addItem(title)
        .then(() => {
          //здесь мы дожидаемся выполнения санки, т.к. это переиспольз-я компонента, мы не можем написать .unwrap, поэтому нужно сюда вернуть реальную санку(для этого в родителе сделали ретурн)
          setTitle("");
        })
